    ...
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [
                ...
                if download.message.chat.type != 'private':
                    try:
                        ...
                        msg += f'\n<b>ℹ️ Source : </b><a href="https://t.me/c/{chatid}/{download.message.message_id}">Click Here</a>'
                    except:
                        pass
                msg += f'\n<b>🙎 </b> ️<code>{download.message.from_user.first_name}</code>️(<code>{download.message.from_user.id}</code>)'
                msg += f"\n<b>🚫 </b><code>/{BotCommands.CancelMirror} {download.gid()}</code>"
            elif download.status() == MirrorStatus.STATUS_SEEDING:
                msg += f"\n<b>💾 Size: </b>{download.size()}"
                msg += f"\n<b>📶 </b>{get_readable_file_size(download.torrent_info().upspeed)}/s"
            ...
            if STATUS_LIMIT is not None and index == STATUS_LIMIT:
                break
        bmsg = f"<b>🖥️ CPU:</b> {cpu_percent()}% | <b>🆓 </b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}"
        ...
        if STATUS_LIMIT is not None and tasks > STATUS_LIMIT:
            msg += f"<b>📑 </b> {PAGE_NO}/{pages} | <b>🚧 Tasks:</b> {tasks}\n"
            buttons = ButtonMaker()
